...
//
//                   TRABALHO FINAL
//          Henrique Trein     Alef Pasqualotto

//Reaproveitando dos labs
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <map>
...
// renderização.
glm::vec4 camera_position = glm::vec4(0.0f, 0.0f, 2.5f, 1.0f); // Posição inicial da câmera
float camera_yaw = 0.0f;   // Rotação horizontal (esquerda/direita)
float camera_pitch = 0.0f; // Rotação vertical (cima/baixo)
float camera_speed = 0.050f; // Velocidade de movimento da câmera
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(1200, 800, "Jogo sem nome", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    glfwSetWindowSize(window, 1200, 800); // Forçamos a chamada do callback acima, para definir g_ScreenRatio.
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_position_c  = camera_position; // Ponto "c", centro da câmera
        ...
        // Vetor "up" permanece sempre o eixo Y global
        glm::vec4 camera_up_vector = glm::vec4(0.0f, 1.0f, 0.0f, 0.0f);

        // Computamos a matriz View usando a nova câmera
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    const float max_pitch = 3.141592f/2.0f - 0.1f;
...
void ScrollCallback(GLFWwindow* window, double xoffset, double yoffset)
{
    // Ajusta a velocidade da câmera com a roda do mouse
    camera_speed += 0.01f * yoffset;
    
    // Limita a velocidade mínima e máxima
    if (camera_speed < 0.01f)
        camera_speed = 0.01f;
    if (camera_speed > 1.0f)
        camera_speed = 1.0f;
}
